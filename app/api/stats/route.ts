...
export async function GET() {
  ...
  const stats = players.map((p) => {
    ...
    const directFaults = p.faults.length;
    let sharedFaults = 0;
    ...
    for (const game of games) {
      ...
      for (const t of game.throws) {
        if (t.sharedFault && opposingIds.has(t.throwerId)) sharedFaults++;
    ...
    const totalFaults = directFaults + sharedFaults;
    const totalDefenses = totalCatches + totalFaults;
